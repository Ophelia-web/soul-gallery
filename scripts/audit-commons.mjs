...
const SUSPECT =
  /\b(crop|cropped|detail|study|copy|after|poster|frame|installation|museum room)\b/i;
const BATCH_SIZE = 20;

function normalizeFileTitle(fileTitle) {
  const trimmed = String(fileTitle || '').trim();
  if (!trimmed) throw new Error('Missing Wikimedia Commons file title');
...
async function fetchJson(url, attempt = 0) {
  ...
  if (response.status === 429 || text.startsWith('You are making too many requests')) {
    if (attempt >= 8) throw new Error('Wikimedia request failed: 429');
...
async function fetchImageInfoBatch(fileTitles) {
  ...
  for (const requested of fileTitles) {
    ...
    if (!page || page.missing || !page.imageinfo?.[0]) {
      map.set(requested, { error: `Exact Wikimedia Commons file was not found: ${requestedTitle}` });
      continue;
    }
...
function validateInfo(artwork, returnedTitle, info) {
  ...
  if (SUSPECT.test(artwork.commonsFile) || SUSPECT.test(returnedTitle)) {
    failures.push('filename contains suspect term (crop/detail/copy/etc.)');
  }

  if (!ALLOWED_MIME.has(info.mime)) failures.push(`unsupported mime ${info.mime}`);

  const width = Number(info.width || 0);
  const height = Number(info.height || 0);
  if (!width || !height) failures.push('missing dimensions');
...
function printRow(result) {
  const size = result.width && result.height ? `${result.width}×${result.height}` : 'n/a';
...
function toMarkdown(results, structuralFailures) {
  ...
  for (const result of results) {
    ...
    lines.push(`- Aspect ratio: ${result.aspectRatio == null ? 'n/a' : result.aspectRatio}`);
    lines.push(`- MIME: ${result.mime || 'n/a'}`);
    lines.push(`- Source: ${result.source ? `[Commons page](${result.source})` : 'n/a'}`);
...
if (missingFiles.length) {
  structuralFailures.push(`Missing commonsFile: ${missingFiles.map((item) => item.id).join(', ')}`);
}
...
for (let index = 0; index < artworks.length; index += BATCH_SIZE) {
  ...
  if (index + BATCH_SIZE < artworks.length) await sleep(1200);
}
...
writeFileSync(new URL('../IMAGE_AUDIT.md', import.meta.url), toMarkdown(results, structuralFailures));
console.log('Wrote IMAGE_AUDIT.md');

if (failed.length || structuralFailures.length) process.exit(1);
